...
import (
	...
	"github.com/Mayank85Y/boil/internal/sqlerr"
...
func (global *GlobalMiddlewares) GlobalErrorHandler(err error, c echo.Context){
	//first try to handle database err and onvent ot properr http error
	originalErr := err

	//first handle known databaser , only those err that haven;t converted to HTTp err
	var httpErr *errs.HTTPError
	if !errors.As(err, &httpErr){
		...
		if errors.As(err, &echoErr){
			...
			//sqlerr will convert database errs to appropitar app err
